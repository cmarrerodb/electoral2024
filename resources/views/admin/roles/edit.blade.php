...
@section('content')
    <form action="{{ route('admin.roles.update', $role) }}" method="PUT">
        ...
        @foreach ($permissions as $permission)
            <div>
                <input type="checkbox" name="permissions[]" id="{{$permission->id}}" class="mr-1" {{ in_array($permission->id, array_column($permisos, 'permission_id')) ? 'checked' : '' }}>{{$permission->description}}                
            </div>
        @endforeach        
        <!-- ----------------------------------------- -->
